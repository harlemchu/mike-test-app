...
class ReorderableFacultyListManager extends StatefulWidget {
  final String department;
  const ReorderableFacultyListManager({super.key, required this.department});
  @override
  _ReorderableFacultyListManagerState createState() =>
      _ReorderableFacultyListManagerState();
}

class _ReorderableFacultyListManagerState
    extends State<ReorderableFacultyListManager> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.department.toString()),
      ),
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance
            .collection('faculty')
            .where('department', isEqualTo: widget.department.toString())
            .orderBy('id')
            .snapshots(),
          ...
          return Container(
            ...
            child: ReorderableListView(
              ...
              children: _docs.map((doc) {
                ...
                // ListTile(
                //   key: ValueKey(doc.id),
                //   title: Text(doc['position'].toString() ??
                //       'No Name'), // Adjust based on your document structure
                //   subtitle: Text('${doc['name'].toString()}'),
                // );

                //Adding picture in ClipRRect widget with CircularProgressIndicator | By Edison S. Tanza
              }).toList(),
            ),
          );
        },
